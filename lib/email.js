...
const emailRegistro = async (datos) => {
  ...
  const link = `http://localhost:4000/auth/confirma/${token}`;
  ...
  await transport.sendMail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; line-height:1.5;">
        <h2 style="color:#f59e0b;">BienesRaices_240136</h2>

        <p>Hola <b>${nombre}</b>, comprueba tu cuenta en <b>BienesRaices_240136</b>.</p>
        <hr>
...
const emailResetearPassword = async (datos) => {
  ...
  const link = `http://localhost:4000/auth/actualizarPassword/${token}`;
