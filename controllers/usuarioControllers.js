...
export const formularioActualizacionPassword = (req, res) => {
...
export const autenticarUsuario = async (req, res) => {
  ...
  try {
    ...
    if (typeof usuario.validarPassword !== "function") {
      return res.render("auth/login", {
        pagina: "Inicia sesión",
        errores: [{ msg: "Falta el método validarPassword en el modelo Usuario" }],
        mensaje: null,
        datos: { emailUsuario: email },
      });
    }
...
export const solicitarRecuperacionPassword = async (req, res) => {
  ...
  try {
    ...
    return res.render("auth/recuperarPassword", {
      pagina: "Te ayudamos a restaurar tu contraseña",
      errores: [],
      ok: "Listo. Revisa tu correo (o la consola para ver el token).",
      datos: { emailUsuario: email },
    });
  } catch (error) {
    console.error(error);
...
export const resetearPassword = async (req, res) => {
  ...
  try {
    const usuario = await Usuario.findOne({
      where: { email: usuarioSolicitante?.trim().toLowerCase() },
    });
    ...
    usuario.tokenRecovery = generarToken();
    usuario.tokenExpiration = new Date(Date.now() + 60 * 60 * 1000);
    await usuario.save();
